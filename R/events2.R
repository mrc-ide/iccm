...
create_event_listeners <- function(events, variables, parameters, renderer){
  ...
  if(FALSE){
      ...
      events[[event]]$add_listener(
        onset_symptoms(
          variable = variables[[paste0(parameters$disease[[disease]]$type, "_symptom_onset")]]
        )
      )
...
schedule_progression_from_severe_infection <- function(parameters, progress_to_asymptomatic_infection, progress_to_uninfected){
  function(timestep, target){
